...
router.get('/editContact/:id', (req, res, next) => {
  thecontact = contacts.find(c => c.id === Number(req.params.id));
  res.render('layout', {
    title: 'edit Contact',
    partials: {
      content: 'editContact'
...
router.post('/editContact/:id', (req, res, next) => {
  thecontact = contacts.find(c =>  Number(req.params.id) === c.id);
  console.log(thecontact);
  thecontact.fist = req.body.first;
  thecontact.last = req.body.last;
  thecontact.email = req.body.email;
  thecontact.phone = req.body.phone;
