...
(async function () {
  ...
  const descriptionElem = $('#descriprion');

  const instuctionsElem = $('#instructions');
  ...
  async function selectRecipe(e) {
    console.log(e.target.value)
    const recipe = await loadJson(`/recipe-api/${e.target.value}`);
    if (! recipe) {
      return;
    }
    noRecipe.hide();
  ...
  const recipes = await loadJson('/recipe-api');
